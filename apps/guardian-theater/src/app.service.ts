  ...
  async getAllEncounteredVideos(
    ...
    if (membershipType === 254) {
      ...
      if (linkedProfiles.Response?.bnetMembership) {
        const childProfiles: DestinyProfile[] = [];
        let dateLastPlayed: Date;
        for (let j = 0; j < linkedProfiles.Response.profiles.length; j++) {
          ...
          const childProfile = {
            ...
            timestamps: {
              pageLastVisited: new Date(),
              bnetProfileChecked: new Date(),
            },
          };
        ...
        await this.firestoreService.updateDestinyProfiles(childProfiles);
      }
    } else {
      ...
      if (userInfo) {
        ...
        await this.firestoreService.updateDestinyProfiles([updateProfile]);
      }
      const profileRes = await this.firestoreService.getDestinyProfile(
        destinyMembershipId,
      );
      profile = profileRes as DestinyProfile;
      if (profile.bnetProfile && profile.bnetProfile.membershipId) {
        const profilesRes = await this.firestoreService.getDestinyProfileByBnetMembershipId(
          profile.bnetProfile.membershipId,
        );
        for (let i = 0; i < profilesRes.length; i++) {
          const membershipId = profilesRes[i].id;
          membershipIds.push(membershipId);

          const updates = [];

          if (membershipId !== destinyMembershipId) {
            const updateProfile = profilesRes[i] as DestinyProfile;
            updateProfile.membershipId = membershipId;
            updateProfile.timestamps.pageLastVisited = new Date();
            updates.push(updateProfile);
          }

          if (updates.length) {
            await this.firestoreService.updateDestinyProfiles(updates);
          }
        }
      } else {
